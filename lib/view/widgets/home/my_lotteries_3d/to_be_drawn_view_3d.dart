...
class ToBeDrawnView3D extends ConsumerStatefulWidget {
  final int categoryId;
  const ToBeDrawnView3D({Key? key, required this.categoryId}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _ToBeDrawnView3DState createState() => _ToBeDrawnView3DState();
}

class _ToBeDrawnView3DState extends ConsumerState<ToBeDrawnView3D> {
                        ...
                        child: DataTable2(
                          ...
                          rows: List<DataRow>.generate(
                              ...
                              (index) => DataRow(cells: [
                                    ...
                                    DataCell(Center(
                                        child: Text(results[index].barCode))),
                                    DataCell(
                                      Center(
  ...
  void searchResult() async {
    ...
    if (networkStatus) {
      results.clear();
      debugPrint("checing game id ${ref.watch(filterMyLotteriesProvider)}");
  ...
  void onCheckPrint(
    ...
    String address = 'Retailer: $userName\r\nDraw Date: ${results[index].time}';
    int totalTicketNo = 0;
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (final context) => [
        ...
        pw.ListView.builder(
          itemCount: results[index].ticketNo.length,
          itemBuilder: (context, i) {
            ...
            for (int j = 0;
              ...
              rows.add(
                  numbers.sublist(j, (j + newNumber).clamp(0, numbers.length)));
            }
  ...
  //                   totalPriceSeperated =
  //                       (ticketNo.price) * totalQuantitySeperated;
