...
class DrawnView2D extends ConsumerStatefulWidget {
  final int categoryId;
  const DrawnView2D({required this.categoryId, Key? key}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _DrawnView2DState createState() => _DrawnView2DState();
}

class _DrawnView2DState extends ConsumerState<DrawnView2D> {
                                  ...
                                  children: [
                                    Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                            "Play Points : ${results[index].ticketPrice ?? "-"}"),
                                        const SizedBox(
                                          width: 10,
                                        ),
  ...
  void searchResult() async {
    ...
    if (networkStatus) {
      ...
      if (ref.watch(filterMyLotteriesProvider).isNotEmpty) {
        ...
        await ref
            .watch(drawnProvider2d(UserResultsParams(
          claimStatus: 0,
