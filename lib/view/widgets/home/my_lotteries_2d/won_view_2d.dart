...
class WonView2D extends ConsumerStatefulWidget {
  final int categoryId;
  const WonView2D({Key? key, required this.categoryId}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _WonView2DState createState() => _WonView2DState();
}

class _WonView2DState extends ConsumerState<WonView2D> {
  // DateTime now =
...
// Show DateRangePicker
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          "Barcode: ${results[index].barCode ?? "_"}",
                                        ),
                                        Text(
                                          "Play Points: ${results[index].ticketPrice!.floor()}",
                                        ),
                                        Text(
                                          "Win Points: ${results[index].winPrice!.floor()}",
                                        ),
                                        Text(
                                          "Jackpot Win Points: ${results[index].jackpotPrice!.floor()}",
                                        ),
                                        Text(
                                          "Total Win Points: ${results[index].totalWinPrice!.floor()}",
                                        ),
                                          ...
                                          onPressed: () async {
                                            ...
                                            Navigator.push(
                                              ...
                                              MaterialPageRoute(
                                                builder: (context) =>
                                                    TicketViewPage(
                                                  showWinNoHeader: true,
                                                  playPoints: results[index]
                                                      .ticketPrice!,
                                                  winPrice:
                                                      results[index].winPrice!,
                                                  jackpotPrice: results[index]
                                                      .jackpotPrice!,
                                                  totalWinPrice: results[index]
                                                      .totalWinPrice!,
                                                  gameId:
  ...
  void searchResult() async {
    ...
    if (networkStatus) {
      ...
      if (ref.watch(filterMyLotteriesProvider).isNotEmpty) {
        ...
        await ref
            .watch(wonProvider2d(UserResultsParams(
  ...
  Future<void> wonProviderCallFunction() async {
    ...
    ref
        .refresh(wonProvider2d(userResultSearchParams).future)
        .then((value) => setState(() {
              results = value.results!;
            }))
        .onError((error, stackTrace) {
